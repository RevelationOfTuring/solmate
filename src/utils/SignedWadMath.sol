...
function wadExp(int256 x) pure returns (int256 r) {
    unchecked {
        ...
        // 2. 基数转换：从 10^18 定点转为 2^96 二进制定点数
...
function wadLn(int256 x) pure returns (int256 r) {
    unchecked {
        ...
        // 3.3 r × ln(2) 怎么补回？
        //   由 3.1 的分解：ln(x) = r × ln(2) + ln(m)，代入 r = k + 96：
        //   ln(x) = (k + 96) × ln(2) + ln(m)
        ...
        //      1. 96 × ln(2)：(k + 96) × ln(2) 中 k 份已显式补回，这里补剩余的 96 份
        //      2. -ln(10^18)：输入 x 是 wad 定点，目标是 ln(x/10^18)，需减掉多出的 ln(10^18)
        // 5.3 代码实现：在 5^18 × 2^192 基数下统一计算
        //  数学目标：result_wad = (r × s + k × ln(2) + ln(2^96 / 10^18)) × 10^18
        //  但不能直接乘 10^18（r 在 2^96 基数，直接乘会损失精度或溢出）
        //  技巧：10^18 = 5^18 × 2^18，选 5^18 × 2^192 作为中间基数，
        //  算完后只需 >> 174（= 192 - 18）就能转回 wad（5^18 × 2^18 = 10^18）
